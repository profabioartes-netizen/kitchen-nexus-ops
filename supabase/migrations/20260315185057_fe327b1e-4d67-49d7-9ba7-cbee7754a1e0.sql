...
BEGIN
  ...
  -- Log de criação de comanda manual
  ...
  VALUES (
    ...
    created_order.id,
    'comanda_created',
    format('Comanda criada | tipo=manual | order_id=%s | mesa_id=%s | session_id=null', created_order.id, p_table_id),
...
BEGIN
  ...
  IF NOT v_is_reuse THEN
    ...
    -- Log de criação de comanda autoatendimento
    INSERT INTO public.table_activity_log (
      table_id, order_id, action, description, user_name
    )
    VALUES (
      ...
      v_created_order.id,
      'comanda_created',
      format('Comanda criada | tipo=autoatendimento | order_id=%s | mesa_id=%s | session_id=%s', v_created_order.id, p_table_id, p_session_id),
