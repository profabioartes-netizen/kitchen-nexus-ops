            ...
            <div className="flex flex-col">
              {order ? (
                <input
                  type="text"
                  defaultValue={order.customer_name || ""}
                  key={`d-cust-${order.id}-${order.customer_name ?? ""}`}
                  placeholder={(table as any)?.default_name || table?.name || "Cliente"}
                  onBlur={async (e) => {
                    const newName = e.target.value.trim();
                    if (newName !== (order.customer_name || "")) {
                  ...
                  onKeyDown={(e) => { if (e.key === "Enter") (e.target as HTMLInputElement).blur(); }}
                  className="text-xl font-semibold leading-tight bg-transparent border-b border-transparent hover:border-border focus:border-ring outline-none py-0.5 max-w-[260px]"
                />
              ) : (
