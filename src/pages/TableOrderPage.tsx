  ...
  const printBill = useMutation({
    mutationFn: async () => {
      ...
      await supabase.from("print_jobs").insert({
        ...
        payload: {
          type: "bill",
          location: (order as any).current_location || table?.internal_number || table?.default_name || (order as any).origin_location || "—",
          table_name: (order as any).current_location || table?.internal_number || table?.default_name || (order as any).origin_location || "—",
        ...
        for (const [station, stationItems] of Object.entries(itemsByStation)) {
          printJobRows.push({
            ...
            payload: {
              items: stationItems,
              table_name: (order as any).current_location || table?.internal_number || table?.default_name || (order as any).origin_location || "—",
              location: (order as any).current_location || table?.internal_number || table?.default_name || (order as any).origin_location || null,
              customer_name: order.customer_name || null,
              waiter_name: order.waiter_name || null,
              origin: order.origin || "waiter",
              order_id: order.id,
          ...
          {order?.status === "paid_pending_finalization" ? (
            ...
            <PaymentPanel
              ...
              orderContext={order ? {
                ...
                origin: order.origin || "waiter",
                location: (order as any).current_location || table?.internal_number || table?.default_name || (order as any).origin_location || "—",
                tableName: (order as any).current_location || table?.internal_number || table?.default_name || (order as any).origin_location || "—",
              } : undefined}
            />
          )}
        </div>
