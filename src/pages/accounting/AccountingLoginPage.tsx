  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profile?.role !== "contabilidade") {
        ...
        toast.error("Acesso restrito à contabilidade.");
        return;
      }
    ...
    <div className="h-screen overflow-hidden flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-sm flex-shrink-0">
        <div className="text-center mb-10">
          <div className="mx-auto mb-4 h-20 w-20 rounded-2xl bg-accent/10 flex items-center justify-center">
            <BarChart3 className="h-10 w-10 text-accent" />
          </div>
          <h1 className="font-display text-2xl tracking-tight">Painel Contábil</h1>
          <p className="text-sm text-muted-foreground mt-1.5 tracking-wide">Coffee Thrones</p>
        </div>
